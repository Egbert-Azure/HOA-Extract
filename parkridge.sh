#!/bin/bash
# Parkridge HOA Extract - one-command setup + run (macOS)
# Safe to run every time: installs what's missing, then processes packets.
set -o pipefail

echo ""
echo "=================================================="
echo "  Parkridge HOA Extract"
echo "=================================================="

# --- Put Homebrew on PATH if it exists (Apple Silicon or Intel) ---
...
if ! command -v brew >/dev/null 2>&1; then
  ...
  echo "  You may be asked to press Return and to type your Mac password."
  echo ""
  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" || {
    echo "  Homebrew install did not complete. Please re-run this command."; exit 1; }
  # re-add to PATH after fresh install
  if [ -x /opt/homebrew/bin/brew ]; then eval "$(/opt/homebrew/bin/brew shellenv)"
  elif [ -x /usr/local/bin/brew ]; then eval "$(/usr/local/bin/brew shellenv)"; fi
fi

# --- Install the two tools if missing (quiet if already present) ---
for tool in tesseract pdftotext; do
  command -v "$tool" >/dev/null 2>&1 || NEED_BREW=1
done
if [ -n "$NEED_BREW" ]; then
  echo ""
  echo "  Installing tools (tesseract, poppler)..."
  brew install tesseract poppler
fi

# --- Set up the working folder ---
WORKDIR="$HOME/Parkridge"
PACKETS="$WORKDIR/packets"
mkdir -p "$PACKETS"

# --- Download the latest script ---
echo ""
echo "  Getting the latest analysis script..."
curl -fsSL "https://raw.githubusercontent.com/Egbert-Azure/HOA-Extract/main/hoa_extract.py" \
     -o "$WORKDIR/hoa_extract.py" \
  || { echo "  Could not download the script. Check internet and re-run."; exit 1; }

# --- Any packets to process? ---
shopt -s nullglob
FILES=("$PACKETS"/*)
if [ ${#FILES[@]} -eq 0 ]; then
  echo ""
  echo "=================================================="
  echo "  SETUP DONE - no packets yet."
  echo ""
  echo "  1. Open Finder, go to your Home folder,"
  echo "     open  Parkridge  >  packets"
  echo "  2. Drop your TMT packet files (PDF or zip) in there"
  echo "  3. Paste this same command again to build reports"
  echo "=================================================="
  open "$PACKETS" 2>/dev/null
  exit 0
fi

# --- Run the analysis ---
echo ""
echo "  Found ${#FILES[@]} packet(s). Analyzing..."
cd "$WORKDIR"
python3 "$WORKDIR/hoa_extract.py" "$PACKETS"/*

# --- Convert each .md report to a formatted, browser-readable page ---
python3 - "$WORKDIR/hoa_extract_out" << 'PYEOF'
...
echo ""
echo "=================================================="
echo "  DONE - opening your reports."
echo "  (also saved in ~/Parkridge/hoa_extract_out)"
echo "=================================================="
open "$WORKDIR/hoa_extract_out/index.html" 2>/dev/null
open "$WORKDIR/hoa_extract_out" 2>/dev/null